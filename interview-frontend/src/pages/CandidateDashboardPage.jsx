      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 page-enter-delay-2">
        <div className="lg:col-span-2 space-y-8">
          ...
          {result && <div className="space-y-8">
            ...
            {result.shortlisted && !interviewCompleted && !finalDecision && <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-8 rounded-3xl text-white"><h3 className="text-xl font-bold font-display mb-2">Schedule Your Interview</h3><p className="text-blue-100 mb-6">Pick a date and time to unlock your interview link.</p><div className="flex flex-col sm:flex-row gap-3"><div className="relative"><Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-700 w-4 h-4" /><input id="interview_date" name="interview_date" type="datetime-local" value={scheduleDate} onChange={(e) => setScheduleDate(e.target.value)} disabled={scheduling} className="pl-10 pr-4 py-3 rounded-2xl text-slate-900 bg-white outline-none min-w-[250px]" /></div><button onClick={handleScheduleInterview} disabled={scheduling} className="px-8 py-3 rounded-2xl bg-white text-blue-600 font-black hover:scale-[1.01] transition-all shadow-xl disabled:opacity-60">{scheduling ? "Scheduling..." : result.interview_date ? "Reschedule" : "Schedule Interview"}</button></div>{result.interview_date && <div className="mt-4 flex flex-wrap gap-3"><p className="text-sm text-blue-100">Scheduled for: <span className="font-bold">{new Date(result.interview_date).toLocaleString()}</span></p><a href={`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent("Interview - " + (selectedJd?.title || "Quadrant Technologies"))}&dates=${new Date(result.interview_date).toISOString().replace(/[-:]/g, '').split('.')[0]}Z/${new Date(new Date(result.interview_date).getTime() + 60*60*1000).toISOString().replace(/[-:]/g, '').split('.')[0]}Z&details=${encodeURIComponent("Join Link: " + (result.interview_link || ""))}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-3 py-1.5 rounded-lg bg-white/20 hover:bg-white/30 text-sm font-medium transition-colors"><span>📅 Add to Google Calendar</span></a></div>}</div>}
          </div>}
        </div>
